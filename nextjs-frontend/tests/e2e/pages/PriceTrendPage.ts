import { Page, Locator, expect } from '@playwright/test';
import { waitForDataLoad } from '../helpers/navigation';

export class PriceTrendPage {
  readonly page: Page;
  readonly heading: Locator;
  readonly subtitle: Locator;
  readonly regionFilter: Locator;
  readonly startDateInput: Locator;
  readonly endDateInput: Locator;
  readonly groupBySelect: Locator;
  readonly statsCards: Locator;
  readonly chartContainers: Locator;
  readonly lineChart: Locator;
  readonly statsTable: Locator;
  readonly statsTableRows: Locator;
  readonly loadingSkeleton: Locator;
  readonly errorBanner: Locator;
  readonly emptyState: Locator;

  constructor(page: Page) {
    this.page = page;
    this.heading = page.locator('h1', { hasText: '가격 추이 분석' });
    this.subtitle = page.getByText('시간에 따른 아파트 가격 변동을 분석합니다');
    this.regionFilter = page.locator('#region-filter');
    this.startDateInput = page.locator('#start-date');
    this.endDateInput = page.locator('#end-date');
    this.groupBySelect = page.locator('#group-by');
    this.statsCards = page.locator('p.text-2xl.font-bold');
    this.chartContainers = page.locator('.recharts-surface');
    this.lineChart = page.locator('.recharts-line');
    this.statsTable = page.locator('table').first();
    this.statsTableRows = page.locator('table tbody tr');
    this.loadingSkeleton = page.locator('.animate-pulse').first();
    this.errorBanner = page.locator('.bg-red-50');
    this.emptyState = page.getByText('표시할 데이터가 없습니다');
  }

  async goto(): Promise<void> {
    await this.page.goto('/price-trend');
    await waitForDataLoad(this.page);
  }

  async selectRegion(regionValue: string): Promise<void> {
    await this.regionFilter.selectOption(regionValue);
    await waitForDataLoad(this.page);
  }

  async setStartDate(value: string): Promise<void> {
    await this.startDateInput.fill(value);
    await waitForDataLoad(this.page);
  }

  async setEndDate(value: string): Promise<void> {
    await this.endDateInput.fill(value);
    await waitForDataLoad(this.page);
  }

  async setGroupBy(value: 'month' | 'quarter'): Promise<void> {
    await this.groupBySelect.selectOption(value);
    await waitForDataLoad(this.page);
  }

  async getStatsCardCount(): Promise<number> {
    return await this.statsCards.count();
  }

  async getChartCount(): Promise<number> {
    return await this.chartContainers.count();
  }

  async getTableRowCount(): Promise<number> {
    return await this.statsTableRows.count();
  }

  async expectLoaded(): Promise<void> {
    await expect(this.heading).toBeVisible();
    await expect(this.subtitle).toBeVisible();
    await expect(this.errorBanner).toHaveCount(0);
  }

  async expectDataDisplayed(): Promise<void> {
    await expect(this.emptyState).toHaveCount(0);
    const statsCount = await this.getStatsCardCount();
    expect(statsCount).toBeGreaterThanOrEqual(4);
    const chartCount = await this.getChartCount();
    expect(chartCount).toBeGreaterThanOrEqual(1);
  }

  async expectFiltersVisible(): Promise<void> {
    await expect(this.regionFilter).toBeVisible();
    await expect(this.startDateInput).toBeVisible();
    await expect(this.endDateInput).toBeVisible();
    await expect(this.groupBySelect).toBeVisible();
  }
}
