# API 2: 아파트 매매 실거래가 자료

## 개요
국토교통부에서 제공하는 아파트 매매 실거래가 공개 자료 API입니다. 아파트 매매 거래 정보를 조회할 수 있습니다.
...
### cURL
```bash
curl "https://apis.data.go.kr/1613000/RTMSDataSvcAptTrade/getRTMSDataSvcAptTrade?serviceKey=YOUR_SERVICE_KEY&LAWD_CD=11680&DEAL_YMD=202401&numOfRows=10&pageNo=1"
```
...
const url = `https://apis.data.go.kr/1613000/RTMSDataSvcAptTrade/getRTMSDataSvcAptTrade?serviceKey=${serviceKey}&LAWD_CD=${lawdCd}&DEAL_YMD=${dealYmd}&numOfRows=${numOfRows}&pageNo=${pageNo}`;
...
- 데이터 제공 주기: 매월 업데이트
- 이 API는 일반 매매 실거래가를 제공하며, 상세 정보가 필요한 경우 API 3번(상세 자료)을 사용하세요.
