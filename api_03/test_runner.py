...
class TestRunner:
        ...
        if result.get('error'):
            ...
            if result.get('items'):
                ...
                for idx, item in enumerate(result.get('items', [])[:3], 1):
                    ...
                    print(f"  [{idx}] {apt_name} - {deal_amount}만원")
    ...
    def generate_report(self):
        ...
        # 마크다운 보고서 생성
        report_content = f"""# API 03 테스트 결과 보고서

## 테스트 정보
- **테스트 일시**: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
- **API**: 아파트 매매 실거래가 상세 자료
...
"""
                        ...
                        # 상세 정보 추가
                        if item.get('도로명') or item.get('roadNm'):
                            report_content += f"- **도로명**: {item.get('도로명') or item.get('roadNm', 'N/A')}\n"
                        if item.get('해제여부'):
                            report_content += f"- **해제여부**: {item.get('해제여부')}\n"
                        if item.get('해제사유발생일'):
                            report_content += f"- **해제사유발생일**: {item.get('해제사유발생일')}\n"
...
def main():
    ...
    test_cases = [
        ...
        },
        {
            ...
            'lawd_cd': '11680',
            'deal_ymd': '202312',
            'description': '강남구 매매 상세 거래 데이터 조회'
        },
        {
            'name': '강남구 2023년 11월',
            'lawd_cd': '11680',
            'deal_ymd': '202311',
            'description': '강남구 2023년 11월 상세 데이터 조회'
        },
        {
            'name': '서초구 2023년 12월',
            'lawd_cd': '11650',
            'deal_ymd': '202312',
            'description': '서초구 매매 상세 거래 데이터 조회'
        },
        {
            'name': '송파구 2023년 12월',
            'lawd_cd': '11710',
            'deal_ymd': '202312',
            'description': '송파구 매매 상세 거래 데이터 조회'
        }
    ]
