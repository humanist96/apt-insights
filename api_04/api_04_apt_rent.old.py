...
class AptRentAPI:
    ...
    @staticmethod
    def parse_rent_amount(amount: str, rent_type: str) -> Dict:
        """
        전세/월세 금액 파싱
        
        Args:
            amount: 거래금액 문자열
            rent_type: 거래유형 ('전세' 또는 '월세')
        
        Returns:
            파싱된 금액 정보 (deposit: 보증금, monthly: 월세)
        """
        if rent_type == '전세':
            return {
                'deposit': int(amount.replace(',', '')) * 10000 if amount else 0,
                'monthly': None
            }
        elif rent_type == '월세':
            if '/' in amount:
                deposit_str, monthly_str = amount.split('/')
                return {
                    'deposit': int(deposit_str.replace(',', '')) * 10000 if deposit_str else 0,
                    'monthly': int(monthly_str.replace(',', '')) * 10000 if monthly_str else 0
                }
            else:
                return {
                    'deposit': int(amount.replace(',', '')) * 10000 if amount else 0,
