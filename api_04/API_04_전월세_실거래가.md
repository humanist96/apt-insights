# API 4: 아파트 전월세 실거래가 자료

## 개요
국토교통부에서 제공하는 아파트 전월세 실거래가 공개 자료 API입니다. 아파트 전세 및 월세 거래 정보를 조회할 수 있습니다.
...
### cURL
```bash
curl "https://apis.data.go.kr/1613000/RTMSDataSvcAptRent/getRTMSDataSvcAptRent?serviceKey=YOUR_SERVICE_KEY&LAWD_CD=11680&DEAL_YMD=202401&numOfRows=10&pageNo=1"
```
...
const url = `https://apis.data.go.kr/1613000/RTMSDataSvcAptRent/getRTMSDataSvcAptRent?serviceKey=${serviceKey}&LAWD_CD=${lawdCd}&DEAL_YMD=${dealYmd}&numOfRows=${numOfRows}&pageNo=${pageNo}`;
...
| 필드명 | 타입 | 설명 |
|--------|------|------|
| 거래금액 | string | 거래금액 (만원 단위, 쉼표 제거)<br>- 전세: 보증금 (만원)<br>- 월세: 보증금/월세 (만원/만원) |
| 건축년도 | string | 건축년도 |
| 년 | string | 계약년도 |
| 법정동 | string | 법정동명 |
| 아파트 | string | 아파트명 |
...
| 층 | string | 층수 |
| 거래유형 | string | 거래유형 (전세/월세) |

### 거래금액 필드 상세 설명
- **전세**: 보증금만 표시 (만원 단위)
- **월세**: "보증금/월세" 형식으로 표시 (예: "5000/50" = 보증금 5,000만원, 월세 50만원)
- 실제 금액은 이 값에 10,000을 곱한 값입니다.
...
6. **거래금액 파싱**: 
   - 전세의 경우: 거래금액 값 그대로 사용
   - 월세의 경우: "/"로 분리하여 보증금과 월세를 구분
7. **거래유형**: "전세" 또는 "월세"로 구분됩니다.

## 데이터 처리 예시

### JavaScript에서 월세 금액 파싱
```javascript
function parseRentAmount(amount, type) {
  if (type === '전세') {
    return {
      deposit: parseInt(amount) * 10000, // 만원 -> 원
      monthly: null
    };
  } else if (type === '월세') {
    const [deposit, monthly] = amount.split('/');
    return {
      deposit: parseInt(deposit) * 10000, // 만원 -> 원
      monthly: parseInt(monthly) * 10000  // 만원 -> 원
    };
  }
  return null;
}
...
### Python에서 월세 금액 파싱
```python
def parse_rent_amount(amount, rent_type):
    if rent_type == '전세':
        return {
            'deposit': int(amount) * 10000,  # 만원 -> 원
            'monthly': None
        }
    elif rent_type == '월세':
        deposit, monthly = amount.split('/')
        return {
            'deposit': int(deposit) * 10000,  # 만원 -> 원
            'monthly': int(monthly) * 10000   # 만원 -> 원
        }
    return None
```
...
- 데이터 제공 주기: 매월 업데이트
- 전세와 월세 데이터가 함께 제공되므로, 거래유형 필드를 확인하여 구분해야 합니다.
